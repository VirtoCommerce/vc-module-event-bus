...
Alternatively, you can use Event Grid with Logic Apps to process data anywhere, without writing code.
...
To connect Azure Event Grid, you will need setup:
...
To setup a subscription with Azure Event Grid you first need to create a Topic in the [Azure Portal](https://azure.microsoft.com/en-us/services/event-grid/). To allow the Virto Commmerce platform to push messages to your Topic, you need to provide an access key. These can also be found in the Azure Portal  after creating the Topic in the section Access Keys.
...
As an API Client, I want to Add Event Subscription, so that I can receive a set of the events in Event Provider.
...
As an API Client, I want to Update Event Subscription, so that I can actualize set of the events.
...
As an API Client, I want to Remove Event Subscription, so that I can stop receiving events.
...
### View health status
A subscription has `status` and `errorMessage` properties which helps to identify the problem.

!!! note
    This property keep status of the last operation. Use Azure Application Insights to monitor and review historical data.
...
* `Provider` - String - Required - Provider Name. Ex: AzireEventGrid
* `ConnectionString` - String - Required - Connection string for specific provider. Ex: The URI of the Topic.
* `Status` - Int - Optional - Subscription health status. Http response status code from last operation for specific provider. Ex: 200.
* `ConnectionString` - String - Optional - Error message from last operation.
* `SubscriptionEvent` - String - Required - Array of of the events.
